...
public class CodeEditor implements CompetitionEventListener, MemoryEventListener, IBreakpointCheck {
    ...
    public void onMemoryWrite(RealModeAddress address, byte value) {
        ...
        if (absAddr < War.ARENA_SEGMENT * RealModeAddress.PARAGRAPH_SIZE
                || absAddr >= War.ARENA_SEGMENT * RealModeAddress.PARAGRAPH_SIZE + War.ARENA_SIZE)
            return;
        ...
        if (existing == m_fillCmd) {
            ...
            do {
                // rewriting only a single opcode so its not possible to cross to a new opcode
                // which will need reparsing
    ...
    public static class LstLine {
        ...
        public PlayersPanel.Breakpoint tmp_br = null; // used when initializing debug view (doesn't hold info when
                                                      // editing)
    }

    public static final int FLAG_UNPARSED = 1; // means this is a DbgLine of a value written by a warrior and not yet
                                               // parsed by the disassembler
    public static final int FLAG_DEFINE_CODE = 2; // line that came from the user typed text that defines a number (db
                                                  // 123)
    ...
    public static final int FLAG_LSTLINE = 0x07ff0000; // 5 - upper 12 bits of the flat is a 1-based line number of the
                                                       // LstLine that created this DbgLine or 0 if there isn't one
    public static final int FLAG_PLAYER_NUM_SHIFT = 27;
    public static final int FLAG_PLAYER_NUM = 0xf8000000; // upper 5 bits is the player number, valid only if there is a
                                                          // non-zero LstLine

    // one such object can appear in multiple addresses for instance the comment
    // int3 line
    ...
    private DbgLine[] m_dbglines; // for every address, the line of display in the debugger panel or null if line
                                  // is not displayed
    private PlayersPanel.Breakpoint[] m_dbgBreakpoints; // for every address, reference to a Breakpoint object if one
                                                        // exists
    ...
    private PageInfo[] m_pages; // marks when a page of 500 addresses should be redrawn in the next required
                                // time
    ...
    private static byte[] read_file_bin_arr(String name) {
        ...
        int len = arr.length();
        if (len > WarriorRepository.MAX_WARRIOR_SIZE) // anything longer than 512, trim to 512. this is what the
    ...
    // this is used for knowing how many lines there are and placing double click
    // cursor
    ...
    private DocumentFragment makeLineNums(String intext) {
        ...
        // make a new one each time since the old one is kept around for breakpoints
        // line reference
    ...
    private void asm_edit_changed() {
        ...
        // update breakpoints only if there was an editing change (and not when
        // switching displayed code)
    ...
    public void loadedNewBinary(PlayersPanel.Code incode, PlayersPanel callback) {
        ...
        // setting the text will redo the bin according to the disassemler. due to
        // disassembler bugs this may be different
    ...
    // this function analyzes the change made by knowing what was selected or where
    // the caret was
    // and how many lines were removed or added in order to move around breakpoints
    // - it doesn't support dragging text that has NL in it (breakpoint doesn't
    // move)
    private void updateBreakpoints(ArrayList<Integer> prevLineOffsets, int prevLineCount, String prevInText) {
        ...
        // return; // somehow we got here without knowing what's the selection/caret
        // was. should not happe
        // } resetting the selection marker was a bad idea since a drag text move does 2
        // changes immediately one after
        ...
        while (it.hasNext()) {
            ...
            if (br.lineNum != 1) // -1 to make it 0 based, -1 because we want where the previous line ends, +1 to
                                 // move past the NL
                lineStartIndex = prevLineOffsets.get(br.lineNum - 1 - 1) + 1;

            int lineNLIndex = prevLineOffsets.get(br.lineNum - 1); // -1 make it 0 based

            if (selStart != selEnd && (selStart < lineStartIndex && selEnd > lineStartIndex // removed the before the
                                                                                            // start of the line,
                                                                                            // including the start
                    || selStart == lineStartIndex && selEnd > lineNLIndex)) // removed from the start of the line,
                                                                            // passed the end of it
            ...
            // if it's not the above case, we don't care about anything that doesn't change
            // the line count
            if (prevLineCount != m_lineCount) {
                // if we backspaced on a breakpoint, need to remove it
                // need to happen in normal caret, no selection and the backspace on the line of
                // the breakpoint
                if (selStart == selEnd && selStart == lineStartIndex
                        && (keydown == "Backspace" || keydown == "Delete")) {
    ...
    // inspired by https://github.com/kazzkiq/CodeFlask.js#usage which also writes
    // all the dome in every key press
    public void setText(String intext, PlayersPanel playersPanel) {
        ...
        if (!stdout.isEmpty()) { // add coloring to the text
        ...
        if (retcode != 0) { // error
            ...
            opcodes_edit.innerHTML = linesAsInput(intext); // this is needed because x-scroll hiding relies on the
                                                           // opcode pane to be full
            Console.error("~Assembler error");
            if (playersPanel != null)
                playersPanel.updateAsmResult(false, null, null);
            // setBreakpoints(textBreakpoints); // there isn't going to be a new listing,
            // make the breakpoints marking now
            return;
        }
        ...
        /*
         * if (buf.length > WarriorRepository.MAX_WARRIOR_SIZE) { removed since reading
         * is trimmed String msg = "Code is longer than the maximum allowed " +
         * Integer.toString(WarriorRepository.MAX_WARRIOR_SIZE) + " bytes";
         * Console.err_box(msg); asm_output.innerHTML = "<div class='stdout_line_e'>" +
         * msg + "</div>"; if (playersPanel != null) playersPanel.updateAsmResult(false,
         * buf, null); return; }
         */
        ...
        for (int atLstLine = 0; atLstLine < listing.size(); ++atLstLine) {
            ...
            try {
                ...
                if (len != lstline.opcodesCount) {
                    ...
                    // String msgtxt = "disassembled wrong number of bytes " +
                    // Integer.toString(atLstLine+1);
                ...
                Console.error("failed parsing binbuf RuntimeException"); // this should not happen. only happens for
                                                                         // missing cases
            }
    ...
    private void initDebugAreaLines() {
        ...
        for (int i = 0; i < war.getNumWarriors(); ++i) {
            ...
            assert code.lines != null : "unexpected null lines for code " + code.label + " of player"
                    + code.player.getName();
            ...
            // comment or label on the first line, need to belong to the address before
            // first
            if (code.lines.get(0).address == -1) {
                int befFirst = playerLoadOffset - 1;
                if (m_dbglines[befFirst] != null) {
                    last_dbgline = m_dbglines[befFirst];
                    if (last_dbgline == m_fillCmd) { // it's the shared DbgLine from above, make a copy of it since we
            ...
            for (int lsti = 0; lsti < code.lines.size(); ++lsti) {
                ...
                if (lstline.address == -1) {
                    ...
                    last_dbgline.text += "</div><div class='dbg_comment_line'>      <span class='dbg_opcodes'></span>"
                            + lstline.code + "</div>";
                } else {
                    ...
                    /*
                     * dbgline.binOpcode = new byte[lstline.opcodesCount]; -- binOpcode not needed
                     * it seems assert lstline.address + lstline.opcodesCount <= code.bin.length:
                     * "too many opcodes for the program length?"; // copy the binary of this line
                     * to dbgline for later comparison when stepping for(int j = 0; j <
                     * lstline.opcodesCount; ++j) { dbgline.binOpcode[j] = code.bin[lstline.address
                     * + j]; }
                     */

                    if (lsti <= FLAG_LSTLINE_MAX) {// lines above 2^16 are not tracked... should not come to this but
                                                   // just to be safe
    ...
    // dfXXXXX is just the first line that is not a comment - markable by debugger
    // when stepping
    ...
    public void renderLine(int addr, DbgLine dbgline) {
        ...
        if ((dbgline.flags & FLAG_HAS_COMMENT) != 0) // this div tag is closed inside dbgline.text before the comment
                                                     // starts
            dline.innerHTML = "<div id='df" + addrstr + "'><span id='da" + addrstr + "'>" + addrhex + "</span>  "
                    + dbgline.text;
        else
    ...
    // breakpoints in the debug view that are in addresses that don't correspond to
    // code lines are transient.
    // they disappear once the debug session is over. They are not saved in the
    // players m_breakpoints since it's unknown what players are they of
        ...
        if (lsti >= 1) {
            ...
            if (wasAdded) {
                ...
                for (PlayersPanel.Breakpoint exist_br : codeObj.breakpoints)
                    assert exist_br.lineNum != br.lineNum : "Breakpoint of this line already exists! "
                            + Integer.toString(br.lineNum);
            ...
            // add it to the editor as well if needed so it will be visible there when
            // debugging is done
    ...
    public void updateDebugLine() {
        ...
        // line.
        // in this case we don't want to disassemble since the dbglines have not even
        // been inited yet. sort of a hack.
        if (m_dbglines[ipInsideArena] == null) {
            // got to a null line, means this address is hidden and is part of a preceding
            // opcode, first find that
            int opcodeAddr = ipInsideArena;
            while (m_dbglines[opcodeAddr] == null)
                --opcodeAddr;
            // fill the size of this opcode with db lines,
            // do this before disassembly of the IP line to make sure we've erased the old
            // opcode correctly
            ...
            // disassemble may eat at any of the db's after it, and might also each opcode
            // after that
        ...
        if ((m_dbglines[ipInsideArena].flags & FLAG_HAS_COMMENT) != 0)
            ider = "df"; // a line with a comment after, don't highlight the entire line, just the first
                         // line. df is assured to exist if we have this flag
    ...
    private void disassembleAddr(int absaddr, int addrInArea) {
        ...
        for (int i = 1; i < len; ++i) {
            ...
            // don't know what opcodes I'm writing so need to make sure it remains
            // consistant
            eraseOpcode(addrInArea + i);
            // this erases one line and possible adds db in the lines after it, this is
            // simple good although it can write several times in the same place
        }
    }

    // erase the opcode in addr, and take care to setByte the bytes after it that
    // are affected
    ...
    public void setDebugMode(boolean v) {
        if (v) {
            ...
            initDebugAreaLines();
            scrollToCodeInEditor(true); // defer scrolling since we want to do this only after all sizes are correct and
                                        // everything shown
        } else {
